...
$db = mysql_connect("$dbaddress", "$dbuser", "$dbpass");
mysql_select_db("$dbname",$db);
$result = mysql_query("SELECT * FROM nuke_clearchat order by id desc limit 0, $lognumber",$db);


while ($output = mysql_fetch_array($result))
{
echo"<a href=\"chatlog.php?pw=$pw&amp;lognumber=$lognumber&amp;deletenum=";
echo $output["id"];
echo"\" title=\"#";
echo $output["id"];
echo" --- Posted ";
